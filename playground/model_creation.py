...
def classical_ml_model(model, df, file_path, main_folder,suggested_model):
    ...
    messages_2 = [
        ...
        (
            "human",
            "Here is the {code} \n I have preprocessed the data. Make sure to use {main_folder}/ Now Train the model on the data. Save the model trained in '{main_folder}/' folder. Save confusion matrix as png files inside the 'model/' folder. Give back the entire code",
        ),
    ]
    ...
    chain_3 = (
        ...
        | StrOutputParser()
        | RunnableLambda(lambda x: grab_code(x))
        | RunnableLambda(lambda x: create_python_file(x, main_folder, "model"))
...
def create_model(file_path, main_folder, suggested_model):
    ...
    classical_ml_model(model, df, file_path, main_folder, suggested_model)
