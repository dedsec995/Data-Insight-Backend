...
def extract_paths(data, main_folder, sub_folder):
    ...
    for item in data:
        if os.path.join(main_folder, sub_folder) in item:
            path = os.path.relpath(item, main_folder)
            paths.append(path)
...
def generate_visualizations(model, df, file_path, main_folder, visualization_type):
    ...
    if visualization_type == "correlation":
        ...
        sub_folder = "hist"
        message = f"Given the csv. There are {num_columns} number of columns and the {num_rows} number of rows. The file path is {file_path}. The head of csv - {first_five} \n.The target columns is the last column. Write a python file to create different histogram graphs of all features for visualization. Save all the graphs as png files inside the '{os.path.join(main_folder, 'hist')}/' folder. Keep a list of the file paths created and print it. Make sure to use '{os.path.join(main_folder, 'hist')}/' in the file paths when saving the plots."
